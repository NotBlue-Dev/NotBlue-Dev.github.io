...
public static class Program {
    ...
    #region MatchmakerServer

    private static void StartMatchmakerServer(string gameServerIP = "127.0.0.1", short gameServerPort = 8090, bool sendOnce = true) {
        WebSocketServer server = new WebSocketServer("ws://127.0.0.1:8001");
        bool dataAlreadySent = false;
        ...
        server.Start(socket => {
            ...
            socket.OnBinary = bytes => {
                Console.WriteLine($"[MATCHMAKER] DATA RECEIVED: {Convert.ToHexString(bytes)}");

                if (!dataAlreadySent || !sendOnce) {
                    // Getting the hex values for our targeted IP:Port
                    int[] gameServerIP_Split = gameServerIP.Split(".").Select(int.Parse).ToArray();
                    ...
                    string gameServerPort_Hex = gameServerPort.ToString("X");
